...
```bash
cd <skill-path>
python3 scripts/fetch_news.py --source hackernews,github,huggingface --limit 10
```

**关键**：捕获 stdout 输出，不依赖生成的 Markdown 文件（防止缓存污染）。如果某个请求信源返回 0 条，必须在报告顶部显式标注缺失信源，不能假装抓取成功。
...
强制要求：
- 不要只罗列各信源条目，**必须先给“今日概括”**，再展开各信源详情。
- “今日概括”只能基于已抓到的条目做归纳，不得补充原始数据里没有的事实或判断。
- 概括部分优先回答：今天在发生什么、最值得关注什么、有哪些明显趋势或风险。
...
- **一句话总览**：用 1 句话概括今天最明显的信息面，不超过 30 字。
- **重点趋势**：提炼 2-3 个高频主题，例如 AI、开源基础设施、监管、安全。
- **最值得关注**：指出 2-3 条最值得点开的信息，并说明原因。
- **风险/异常**：说明是否存在信源缺失、反爬失败、信息重复、争议性较高等情况。
